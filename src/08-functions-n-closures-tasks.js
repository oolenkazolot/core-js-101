...
function memoize(func) {
  ...
  return () => {
    randomNumber = randomNumber !== null ? randomNumber : func();
    return randomNumber;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const arrArgs = [];
    args.forEach((item) => {
      arrArgs.push(JSON.stringify(item));
    });
    const logStart = `${func.name}(${arrArgs.join(',')}) starts`;
    logFunc(logStart);
    const res = func(...args);
    const logEnd = `${func.name}(${arrArgs.join(',')}) ends`;
    logFunc(logEnd);
    return res;
  };
}
